...
def extract_data_from_pdf(pdf_path):
    ...
    if len(table) == 9:
        ...
        if table[0][0] == "Preferred Name":
            ...
            data["iupac_names"] = [table[2][1]]
            data["inchi_key"] = table[3][1]
            data["cas_num"] = table[4][1]
            data["formula"] = table[5][1]
            data["molecular_mass"] = float(table[6][1])
            data["details"]["molecular_ion_[m+]"] = table[7][1]
            data["details"]["exact_mass_[m+h]+"] = table[8][1]

            re_class = compile(r'(?<=NPS SUBCLASS\n).*?(?=\n)')
            data["categories"] = [re_class.search(text).group(0)]

            data["inchi"] = fetch_opsin_data(data["iupac_names"][0])
        else:
    ...
    data["iupac_names"] = [re_IUPAC_Name.search(text).group(0).replace("\n", "")]
    ...
    if len(table) in [2, 3]:
        ...
        data["molecular_mass"] = float(chemical_data_numbers[0])
        data["details"]["molecular_ion_[m+]"] = chemical_data_numbers[1]
        data["details"]["exact_mass_[m+h]+"] = chemical_data_numbers[-1]
    else:
